<template>
  <table v-bind="extractTableProps()" class="has-item-context-menu">
    <nu-thead v-if="hasHeader">
      <nu-tr class="nu-table-header-row h-10.5">
        <nu-th
          v-for="(field, index) in fields"
          :key="`nu-thead-${field.name}`"
        ...
        >
          <nu-button
            ...
            class="nu-button-sort w-full hover:underline"
            gap-size="small"
            no-hover
          ...
          >
            <span
              ...
              class="nu-table-thead-content inline-table align-middle"
            >
              <slot :name="field.name + 'Header'" />
            </span>
            <span
              v-else
              class="nu-table-thead-content inline-table align-middle header-text"
    ...
    <nu-tbody class="has-item-context-menu">
      <nu-tr
        v-for="(item, trIndex) in data"
        :key="`nu-tbody-tr-${domSelector(item) || trIndex}`"
        :ref="`row-${trIndex}`"
      ...
      >
        <nu-td
          ...
          :key="'nu-tbody-td-' + cellKey(field, tdIndex, item)"
    ...
    <tfoot v-if="$slots.footer" class="nu-table-footer border-t">
      <tr class="nu-table-footer-row h-10.5">
        <td
          :colspan="fullColspan"
          class="nu-table-footer-cell p-1 text-sm text-role-on-surface-variant"
        >
...
const tableClasses = computed(() => {
  const result = ['nu-table']

  if (hover) {
    result.push('nu-table-hover')
  }

  if (sticky) {
    result.push('nu-table-sticky')
  }
...
const extractThProps = (field: FieldType, index: number) => {
  ...
  props.class = `nu-table-header-cell nu-table-header-cell-${field.name}`
...
const extractTbodyTrProps = (item: Item, index: number) => {
  return {
    ...
    class: [
      'nu-tbody-tr',
      `nu-tbody-tr-${domSelector(item) || index}`,
      isHighlighted(item) ? 'nu-table-highlighted' : undefined,
      ...(isDisabled(item)
        ? ['nu-table-disabled', 'opacity-70', 'pointer-events-none', 'grayscale-60']
        : [])
    ].filter(Boolean)
  }
}
...
const extractTdProps = (field: FieldType, index: number, item: Item) => {
  const props = extractCellProps(field)
  props.class = `nu-table-data-cell nu-table-data-cell-${field.name}`
...
@layer components {
  ...
  .nu-table-accentuated,
  .nu-table-highlighted,
  .nu-table .highlightedDropTarget {
    @apply bg-role-secondary-container;
  }
  .nu-table-sticky {
    @apply relative;
  }
  .nu-table-sticky .nu-table-header-cell {
    @apply bg-role-surface sticky z-10;
  }
  .nu-table-hover tr:not(.nu-table-footer-row, .nu-table-header-row, .nu-table-highlighted):hover,
  .nu-button-sort .nu-icon:hover {
