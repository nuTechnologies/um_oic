...
import { ComponentProps, defaultPlugins, shallowMount } from '@xwork-eu/web-test-helpers'
...
describe('OcDatePicker', () => {
  ...
  it('sets the initial date correctly', async () => {
    ...
    await nextTick()
    const inputEl = wrapper.find('.nu-text-input').element as HTMLInputElement
  ...
  it('sets the minimum date correctly', async () => {
    ...
    await nextTick()
    const inputEl = wrapper.find('.nu-text-input')
