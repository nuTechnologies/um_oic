<template>
  <div
    class="nu-modal-background fixed left-0 top-0 z-[var(--z-index-modal)] bg-black/40 flex items-center justify-center flex-row flex-wrap size-full"
  >
    <focus-trap :active="true" :initial-focus="initialFocusRef" :tabbable-options="tabbableOptions">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="nu-modal-title"
        @keydown.esc.stop="cancelModalAction"
      >
        <div
          class="nu-modal-title bg-role-surface-container flex items-center flex-row flex-wrap py-3 px-4 rounded-t-sm"
        >
          <h2 id="nu-modal-title" class="truncate m-0 text-base" v-text="title" />
        </div>
        <div class="nu-modal-body px-4 pt-4">
          <div
            v-if="$slots.content"
            key="modal-slot-content"
            class="nu-modal-body-message mt-2 mb-4"
          >
            <slot name="content" />
          </div>
          <template v-else>
            <p
              v-if="message"
              key="modal-message"
              class="nu-modal-body-message mt-0"
              :class="{ 'mb-0': !hasInput || contextualHelperData }"
              v-text="message"
            />
            <div
              v-if="contextualHelperData"
              class="nu-modal-body-contextual-helper mb-4"
              :class="{ 'mb-0': !hasInput }"
            >
              <span class="text" v-text="contextualHelperLabel" />
              <nu-contextual-helper class="pl-1" v-bind="contextualHelperData" />
            </div>
            <nu-text-input
              v-if="hasInput"
              key="modal-input"
              ...
              class="nu-modal-body-input -mb-5 pb-4"
        ...
        <div v-if="!hideActions" class="nu-modal-body-actions flex justify-end p-4 text-right">
          <div class="nu-modal-body-actions-grid grid grid-flow-col auto-cols-1fr">
            <nu-button
              class="nu-modal-body-actions-cancel"
              :disabled="isLoading"
            ...
            <nu-button
              ...
              class="nu-modal-body-actions-confirm ml-2"
...
const classes = computed(() => {
  return ['nu-modal', 'bg-role-surface', elementClass]
})
