import {
  ...
  shallowMount
} from '@xwork-eu/web-test-helpers'
...
describe('OcTextInput', () => {
  ...
  const selectors = {
    textInputMessage: '.nu-text-input-message span',
    clearInputButton: '.nu-text-input-btn-clear',
    inputField: '.nu-text-input',
    infoIcon: '.nu-text-input-message .nu-icon',
    showPasswordToggleBtn: '.nu-text-input-show-password-toggle',
    copyPasswordBtn: '.nu-text-input-copy-password-button',
    generatePasswordBtn: '.nu-text-input-generate-password-button'
  }
  ...
  describe('when an error message is provided', () => {
    it('should add the error class to the input', async () => {
      ...
      expect(wrapper.find('input').attributes().class).toContain('nu-text-input-danger')
    })
    it('should add the error class to the input message', async () => {
      const wrapper = getShallowWrapper()
      await wrapper.setProps({ modelValue: 'invalid input' })
      await wrapper.setProps({ errorMessage: 'You shall not pass.' })
      expect(wrapper.find(selectors.textInputMessage).classes()).toContain('nu-text-input-danger')
    })
