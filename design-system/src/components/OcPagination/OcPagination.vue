<template>
  <nav class="nu-pagination" :aria-label="$gettext('Pagination')">
    <ol class="nu-pagination-list flex items-center flex-wrap m-0 gap-2">
      <li v-if="isPrevPageAvailable" class="nu-pagination-list-item">
        <router-link
          class="nu-pagination-list-item-prev flex mr-2 rounded-sm hover:bg-role-secondary hover:text-role-on-secondary [&_svg]:hover:!fill-role-on-secondary"
          :aria-label="$gettext('Go to the previous page')"
          :to="previousPageLink"
        >
          <nu-icon name="arrow-drop-left" fill-type="line" color="var(--nu-role-on-surface)" />
        </router-link>
      </li>
      <li v-for="(page, index) in displayedPages" :key="index" class="nu-pagination-list-item">
        <component
          :is="pageComponent(page)"
          class="nu-pagination-list-item-page py-1 px-2 rounded-sm hover:bg-role-secondary hover:text-role-on-secondary transition-colors duration-200 ease-in-out"
      ...
      <li v-if="isNextPageAvailable" class="nu-pagination-list-item">
        <router-link
          class="nu-pagination-list-item-next flex ml-2 rounded-sm hover:bg-role-secondary [&_svg]:hover:!fill-role-on-secondary"
          :aria-label="$gettext('Go to the next page')"
          :to="nextPageLink"
        >
          <nu-icon name="arrow-drop-right" fill-type="line" color="var(--nu-role-on-surface)" />
        </router-link>
      </li>
    </ol>
  </nav>
...
const pageClass = (page: Page) => {
  ...
  if (isCurrentPage(page)) {
    classes.push(
      ...[
        'nu-pagination-list-item-current',
        'font-bold',
        'bg-role-secondary',
    ...
    classes.push('nu-pagination-list-item-ellipsis')
  } else {
    classes.push('nu-pagination-list-item-link')
  }
