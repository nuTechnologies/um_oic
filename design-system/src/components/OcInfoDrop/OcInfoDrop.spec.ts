...
describe('OcInfoDrop', () => {
  ...
  describe('should use props correctly', () => {
    ...
    it('should set a readMore link', () => {
      const wrapper = getWrapperWithProps({ readMoreLink: 'xwork.design' })
      const attributes = wrapper.find('.info-more-link').attributes()
      expect(attributes['href']).toBe('xwork.design')
    ...
    describe('focus trap', () => {
      it('is active if the drop is open', async () => {
        ...
        wrapper.findComponent<typeof OcDrop>('nu-drop-stub').vm.$emit('show-drop')
