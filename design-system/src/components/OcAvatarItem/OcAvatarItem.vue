<template>
  ...
  >
    <span
      class="nu-avatar-item inline-flex items-center justify-center rounded-[50%] bg-center bg-no-repeat bg-size-[18px]"
      :style="{
        backgroundColor,
        width: avatarWidth,
        height: avatarWidth
...
export interface Props {
   ...
   * @default var(--nu-role-on-secondary)
   */
  iconColor?: string
  /**
...
const {
  ...
  icon,
  iconColor = 'var(--nu-role-on-secondary)',
  iconFillType = 'fill',
  iconSize = 'small',
  width = 30
} = defineProps<Props>()
