<template>
  <div
    class="nu-text-input-password-wrapper flex flex-row border rounded-sm"
    :class="{
      'nu-text-input-password-wrapper-danger text-role-error focus:text-role-error border-role-error':
        hasError,
  ...
  >
    ...
    <nu-button
      ...
      v-nu-tooltip="showPassword ? $gettext('Hide password') : $gettext('Show password')"
      :aria-label="showPassword ? $gettext('Hide password') : $gettext('Show password')"
      class="nu-text-input-show-password-toggle px-2"
      appearance="raw"
      size="small"
      @click="showPassword = !showPassword"
    >
      <nu-icon size="small" :name="showPassword ? 'eye-off' : 'eye'" />
    </nu-button>
    <nu-button
      v-if="password && !disabled"
      v-nu-tooltip="$gettext('Copy password')"
      :aria-label="$gettext('Copy password')"
      class="nu-text-input-copy-password-button px-2"
      appearance="raw"
      size="small"
      @click="copyPasswordToClipboard"
    >
      <nu-icon size="small" :name="copyPasswordIcon" />
    </nu-button>
    <nu-button
      v-if="generatePasswordMethod && !disabled"
      v-nu-tooltip="$gettext('Generate password')"
      :aria-label="$gettext('Generate password')"
      class="nu-text-input-generate-password-button px-2"
      appearance="raw"
      size="small"
      @click="generatePassword"
    >
      <nu-icon size="small" name="refresh" fill-type="line" />
    </nu-button>
  </div>
  <portal v-if="showPasswordPolicyInformation" to="app.design-system.password-policy">
    <div class="flex flex-row flex-wrap text-sm pt-2 gap-x-2">
      <div
        ...
        :key="index"
        class="flex items-center nu-text-input-password-policy-rule"
      >
        <nu-icon
          size="small"
          class="mr-1"
          :name="testedRule.verified ? 'checkbox-circle' : 'close-circle'"
          :color="testedRule.verified ? 'var(--nu-role-on-surface)' : 'var(--nu-role-error)'"
        />
        <span
          :class="[{ 'nu-text-input-danger': !testedRule.verified }]"
          v-text="getPasswordPolicyRuleMessage(testedRule)"
        ></span>
        <nu-contextual-helper
