...
describe('OcTextarea', () => {
  ...
  const selectors = {
    textareaMessage: '.nu-textarea-message span',
    textArea: '.nu-textarea'
  }
  describe('id prop', () => {
  ...
  describe('when an error message is provided', () => {
    ...
    it('should add the error class to the textarea', () => {
      expect(wrapper.find('textarea').attributes().class).toContain('nu-textarea-danger')
    })
