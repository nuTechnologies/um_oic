<template>
  <div>
    ...
    <vue-select
      ...
      :clearable="clearable"
      :multiple="multiple"
      class="nu-select bg-transparent"
      :class="{
        'nu-select-position-fixed': positionFixed
      }"
    ...
    >
      ...
      <template #spinner="{ loading: loadingSpinner }">
        <nu-spinner v-if="loadingSpinner" />
      </template>
      <template #selected-option-container="{ option, deselect }">
        <span class="vs__selected" :class="{ 'vs__selected-readonly': option.readonly }">
          <slot name="selected-option" v-bind="option">
            <nu-icon v-if="readOnly" name="lock" class="mr-1" size="small" />
            {{ getOptionLabel(option) }}
          </slot>
          <span v-if="multiple" class="flex items-center ml-2 mr-1">
            <nu-icon v-if="option.readonly" class="vs__deselect-lock" name="lock" size="small" />
            <nu-button
              v-else
              appearance="raw"
      ...
      <template #open-indicator>
        <nu-icon name="arrow-down-s" size="small" />
      </template>
    </vue-select>

    <div
      v-if="showMessageLine"
      class="nu-text-input-message text-sm"
      :class="{
        'nu-text-input-description': !!descriptionMessage,
        'nu-text-input-danger': !!errorMessage
      }"
    >
      <nu-icon
        v-if="!!errorMessage"
...
.vs--disabled {
  ...
  .vs__search,
  .vs__selected {
    background-color: var(--nu-role-surface-container) !important;
    color: var(--nu-role-on-surface) !important;
    pointer-events: none;
  }
...
.nu-select {
  ...
  .vs {
    ...
    &__dropdown-menu {
      ...
      color: var(--nu-role-on-surface);
      background-color: var(--nu-role-surface);
    ...
    &__selected-readonly {
      background-color: var(--nu-role-surface-container-low) !important;
    }
    ...
    &__no-options {
      ...
      &--selected {
        background-color: var(--nu-role-secondary-container);
      }
    }
  ...
  &:focus-within {
    .vs__dropdown-menu,
    .vs__dropdown-toggle {
      border: 1px solid var(--nu-role-surface);
      outline: 2px solid var(--nu-role-outline);
    }
  }
}
