<template>
  <nav
    :id="id"
    :class="`nu-breadcrumb nu-breadcrumb-${variation} overflow-visible`"
    :aria-label="$gettext('Breadcrumbs')"
  >
    <ol class="nu-breadcrumb-list hidden sm:flex items-baseline m-0 p-0 flex-nowrap">
      <li
      ...
      >
        ...
        >
          <span
            :class="[
              ...
              'max-w-3xs',
              {
                'nu-breadcrumb-item-text-last': index === displayItems.length - 1
              }
            ]"
            v-text="item.text"
          />
        ...
        <template v-if="showContextActions && index === displayItems.length - 1">
          <nu-button
            id="nu-breadcrumb-contextmenu-trigger"
            v-nu-tooltip="contextMenuLabel"
            :aria-label="contextMenuLabel"
            appearance="raw"
            no-hover
            class="mx-1"
          >
            <nu-icon name="more-2" color="var(--nu-role-on-surface)" class="align-middle" />
          </nu-button>
          <nu-drop
            drop-id="nu-breadcrumb-contextmenu"
            toggle="#nu-breadcrumb-contextmenu-trigger"
            mode="click"
            close-on-click
    ...
    <nu-button
      ...
      :to="parentFolderTo"
      class="nu-breadcrumb-mobile-navigation sm:hidden block"
    >
      <nu-icon name="arrow-left-s" fill-type="line" size="large" />
    </nu-button>
  </nav>
  <div
    v-if="displayItems.length > 1"
    class="nu-breadcrumb-mobile-current sm:hidden flex justify-center items-center w-0 flex-1"
  >
...
const getBreadcrumbElement = (id: string): HTMLElement => {
  return document.querySelector(`.nu-breadcrumb-list [data-item-id="${id}"]`)
}
...
const dropItemStyling = (
  ...
  const className = 'nu-breadcrumb-item-dragover'
