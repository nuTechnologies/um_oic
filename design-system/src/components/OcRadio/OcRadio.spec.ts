...
import { PartialComponentProps, mount } from '@xwork-eu/web-test-helpers'
...
describe('OcRadio', () => {
  ...
  describe('size prop', () => {
    it.each([
      { size: 'small', expected: 'size-3' },
      { size: 'medium', expected: 'size-4' },
      { size: 'large', expected: 'size-5' }
    ])(
      'should set input class according to the provided size',
      ({ size, expected }: { size: 'small' | 'medium' | 'large'; expected: string }) => {
        const wrapper = getWrapper({ size })
        const radioInput = wrapper.find(radioElementSelector)
        expect(radioInput.attributes('class')).toContain(expected)
      }
    )
  })
  describe('label', () => {
    ...
    describe('hideLabel prop with label', () => {
      it('should have invisible label class if enabled', () => {
        ...
        const labelEl = wrapper.find('label')
        expect(labelEl.exists()).toBeTruthy()
        expect(labelEl.attributes('class')).toContain('sr-only')
      })
      it('should not have invisible label class if disabled', () => {
        const wrapper = getWrapper({ hideLabel: false })
        const labelEl = wrapper.find('label')
        expect(labelEl.exists()).toBeTruthy()
        expect(labelEl.attributes('class')).not.toContain('sr-only')
      })
    })
    describe('disabled prop with label', () => {
      it('should not have cursor pointer label class, if disabled', () => {
        ...
        const labelEl = wrapper.find('label')
        expect(labelEl.exists()).toBeTruthy()
        expect(labelEl.attributes('class')).not.toContain('cursor-pointer')
      })
      it('should have cursor pointer label class, if not disabled', () => {
        const wrapper = getWrapper({ disabled: false })
        const labelEl = wrapper.find('label')
        expect(labelEl.exists()).toBeTruthy()
        expect(labelEl.attributes('class')).toContain('cursor-pointer')
      })
    })
  })
  ...
  describe('input options', () => {
    ...
    const Component = {
      template:
        '<div><nu-radio v-for="o in availableOptions" :key="o" v-model="selectedOption" :option="o" :label="o" /></div>',
      data: () => ({ ...rawData })
    }
    const options = {
      data: () => ({ ...rawData }),
