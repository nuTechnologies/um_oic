<template>
  <span @click="$emit('click', $event)">
    <input
      ...
      type="checkbox"
      name="checkbox"
      class="nu-checkbox m-0.5 border-2 border-role-outline outline-0 focus-visible:outline outline-role-secondary rounded-sm checked:bg-white disabled:bg-role-surface-container-low indeterminate:bg-white bg-transparent inline-block overflow-hidden cursor-pointer disabled:opacity-40 disabled:cursor-default bg-no-repeat bg-center appearance-none align-middle"
      :class="{
        'nu-checkbox-checked bg-white': isChecked,
        'size-3': size === 'small',
        'size-4': size === 'medium',
        'size-5': size === 'large'
      }"
...
.nu-checkbox {
  ...
  &:disabled:checked {
    @include svg-fill($internal-form-checkbox-image, '#000', var(--nu-role-on-surface-variant));
  }
  ...
  &:disabled:indeterminate {
    @include svg-fill(
      ...
      '#000',
      var(--nu-role-on-surface-variant)
    );
  }
}
</style>
