<template>
  <div :class="$attrs.class">
    ...
    <div class="nu-color-input-wrapper relative max-m-5">
      <input
        :id="id"
        v-bind="additionalAttributes"
        type="color"
        :aria-invalid="ariaInvalid"
        class="nu-color-input nu-input rounded-sm py-0.5 focus:border focus:border-role-outline focus:outline-2 focus:outline-role-outline"
        :class="{
          'nu-color-input-danger text-role-error focus:text-role-error border-role-error':
            !!errorMessage,
      ...
      <nu-button
        ...
        class="mr-1 absolute top-[50%] transform-[translateY(-50%)] right-0 nu-color-input-btn-clear"
        appearance="raw"
        no-hover
        @click="onClear"
      >
    ...
    <div
      ...
      class="nu-color-input-message flex items-center text-sm mt-1 min-h-4.5"
      :class="{
        'nu-color-input-description text-role-on-surface-variant relative': !!descriptionMessage,
        'nu-color-input-danger text-role-error focus:text-role-error border-role-error':
          !!errorMessage
      }"
    >
      ...
      <span
        :id="messageId"
        :class="{
          'nu-color-input-description text-role-on-surface-variant flex items-center':
            !!descriptionMessage,
          'nu-color-input-danger text-role-error focus:text-role-error border-role-error':
            !!errorMessage
        }"
