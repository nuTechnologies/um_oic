<template>
  <div
    ...
    class="nu-search flex items-center"
    :class="{ 'nu-search-small': small }"
  >
    <div class="flex-1 relative">
      ...
      >
        <nu-icon v-show="!loading" :name="icon" size="small" fill-type="line" />
        <nu-spinner
          v-show="loading"
    ...
    <div class="nu-search-button-wrapper" :class="{ 'sr-only': buttonHidden }">
      <nu-button
        class="nu-search-button z-0 ml-4 rounded-l-none transform-[translateX(-1px)]"
        appearance="filled"
...
const inputClass = computed(() => {
  ...
  if (!buttonHidden) {
    classes.push(...['nu-search-input-button', 'rounded-r-none'])
  }
  if (small) {
