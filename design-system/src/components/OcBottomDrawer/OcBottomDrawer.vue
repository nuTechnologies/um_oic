<template>
  ...
  >
    <div
      v-if="isOpen"
      ref="bottomDrawerRef"
      class="nu-bottom-drawer-background fixed inset-0 z-[calc(var(--z-index-modal)+2)] top-0 left-0 bg-black/40 size-full"
      role="button"
    ...
    >
      <focus-trap>
        <div
          :id="drawerId"
          tabindex="0"
          class="nu-bottom-drawer fixed inset-x-0 bg-role-surface-container-high rounded-t-sm w-full max-h-[66vh] overflow-y-auto bottom-[-100%] transition-all duration-200 [&.active]:bottom-0"
        >
          <nu-card class="bg-transparent" header-class="flex flex-row justify-between items-center">
            <template #header>
              <nu-button
                v-if="isNestedElement"
                appearance="raw"
                class="raw-hover-surface nu-bottom-drawer-back-button"
                :aria-label="$gettext('Open the parent context menu')"
                @click="openParentDrawer"
              >
                <nu-icon name="arrow-left" fill-type="line" />
              </nu-button>
              <span class="font-semibold" v-text="title" />
              <nu-button
                appearance="raw"
                class="raw-hover-surface nu-bottom-drawer-close-button"
                :aria-label="$gettext('Close the context menu')"
                @click="hide()"
              >
                <nu-icon name="close" fill-type="line" />
              </nu-button>
            </template>
...
@layer components {
  .nu-bottom-drawer .nu-card-body ul:not(:last-child) {
    @apply mb-2;
  }
}
