<template>
  <component :is="tag" class="nu-card">
    <div v-if="hasSlotHeader" class="nu-card-header" :class="headerClass">
      <slot name="header">
        <img
          v-if="logoUrl"
          :src="logoUrl"
    ...
    <div class="nu-card-body" :class="bodyClass">
      <slot />
    </div>
    <div v-if="hasSlotFooter" class="nu-card-footer" :class="footerClass">
      <slot name="footer" />
    </div>
  </component>
</template>
...
@layer components {
  ...
  .nu-card-body > :last-child,
  .nu-card-header > :last-child,
  .nu-card-footer > :last-child {
    @apply mb-0;
  }
}
</style>
