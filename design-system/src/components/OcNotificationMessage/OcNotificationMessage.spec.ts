...
import { defaultPlugins, mount } from '@xwork-eu/web-test-helpers'
...
describe('OcNotificationMessage', () => {
  ...
  describe('status prop', () => {
    it.each(['passive', 'primary', 'success', 'warning', 'danger'])(
      ...
      (status) => {
        ...
        expect(wrapper.attributes('class')).toContain(`nu-notification-message-${status}`)
      }
    )
  ...
  const selectors = {
    messageTitle: '.nu-notification-message-title',
    messageContent: '.nu-notification-message-content',
    messageWrapper: '.nu-notification-message div',
    errorLog: '.nu-error-log',
    errorLogToggleButton: '.nu-notification-message-error-log-toggle-button'
  }
