<template>
  ...
  <div v-else :id="dropId" ref="drop" class="nu-drop shadow-md/20 rounded-sm" @click="onClick">
    <nu-card v-if="$slots.default" :body-class="[getTailwindPaddingClass(paddingSize)]">
      <slot />
    </nu-card>
...
@layer components {
  ...
  .nu-bottom-drawer li a,
  .nu-bottom-drawer li .item-has-switch,
  .nu-bottom-drawer li button:not([role='switch']),
  .nu-drop li a,
  .nu-drop li button:not([role='switch']),
  .nu-drop li .item-has-switch {
    @apply p-2 w-full;
  }
  ...
  .nu-bottom-drawer li:first-child,
  .nu-drop li:first-child {
    @apply mt-0;
  }

  .nu-bottom-drawer li:last-child,
  .nu-drop li:last-child {
    @apply mb-0;
  }
}
