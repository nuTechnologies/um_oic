...
const dom = ({
  ...
  const wrapper = mount(
    {
      template:
        '<div><p id="trigger">trigger</p><nu-drop :position="position" :mode="mode" :padding-size="paddingSize" :enforce-drop-on-mobile="enforceDropOnMobile" toggle="#trigger">show</nu-drop></div>',
      components: { 'nu-drop': Drop }
    },
    {
...
describe('OcDrop', () => {
  ...
  it('handles dropId prop', () => {
    for (let i = 0; i < 5; i++) {
      ...
      expect(wrapper.attributes().id).toBe(`nu-drop-${i + 1}`)
    }
  ...
  it.each(['xsmall', 'small', 'medium', 'large', 'xlarge', 'xxlarge', 'remove'])(
    ...
    async (size: SizeType | 'remove') => {
      ...
      const drop = wrapper.findComponent({ name: 'nu-drop' })
      await nextTick()
  ...
  describe('Component "OcBottomDrawer"', () => {
    it('renders on mobile device', async () => {
      ...
      const { wrapper } = dom()
      await nextTick()
      expect(wrapper.find('nu-bottom-drawer-stub').exists()).toBeTruthy()
    })
    it('does not render on mobile device when "enforceDropOnMobile" is true', async () => {
      ...
      await nextTick()
      expect(wrapper.find('nu-bottom-drawer-stub').exists()).toBeFalsy()
    })
  })
})
