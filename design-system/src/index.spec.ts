...
describe('Depending on what gets passed into the theming options', () => {
  it('Sets correct custom CSS props from theming options', () => {
    ...
    expect(docStyle.getPropertyValue('--nu-role-primary')).toMatch(primary)
    expect(docStyle.getPropertyValue('--nu-role-on-primary')).toMatch(onPrimary)
    expect(docStyle.getPropertyValue('--nu-font-family')).toMatch(options.tokens.fontFamily)
  })
})
