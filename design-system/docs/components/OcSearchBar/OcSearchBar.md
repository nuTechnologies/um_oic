...
Given there is only one instance of `<nu-search-bar>` per page/route, this component should communicate its purpose, being the main site search, to screen readers. If the component serves as a filter form, it is advised to disable the landmark role via `is-filter="true"`.
