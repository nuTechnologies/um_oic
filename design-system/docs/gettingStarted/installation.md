...
## Within an xwork Web app

You don't necessarily need to install the design-system when developing an application or extension for xwork Web, since it's already included in the Web runtime. However, if you want to have proper type support and access to the components in your IDE, or need to use the provided helper functions, you can install the design-system as a dev dependency.

```shell
$ npm install @xwork-eu/design-system --save-dev

$ pnpm add @xwork-eu/design-system -D

$ yarn add @xwork-eu/design-system -D
```
...
```shell
$ npm install @xwork-eu/design-system

$ pnpm add @xwork-eu/design-system

$ yarn add @xwork-eu/design-system
```
...
```ts
import '@xwork-eu/design-system/dist/tailwind.css'
```
...
import DesignSystem from '@xwork-eu/design-system'
...
```ts
/// <reference types="@xwork-eu/design-system/types" />
```

Optionally, you can pass custom design tokens to the design-system. Check the [example theme](https://github.com/xwork-eu/xwork/blob/v3.5.0/services/web/assets/themes/xwork/theme.json) for a list of available tokens.
...
To make sure all icons are loaded properly, you need to make sure they are served by your application. They are located under `node_modules/@xwork-eu/design-system/dist/icons`. It's recommended to copy them to your public folder. You might need to set `iconUrlPrefix: '/'` when installing the design-system to ensure they are always loaded from the correct path.
...
```ts
import { setLanguage } from '@xwork-eu/design-system'
...
```ts
import translations from '@xwork-eu/design-system/dist/translations.json'
```
